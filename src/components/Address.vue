<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in listDataFilter" :class="{'check': checkIndex == index}" @click="checkIndex = index; selectAddressId=item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn" @click="showModelDel(item)">
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" @click="expand" :class="{'open': limit > 3 && this.listData.length > 3}">
                more
                <i class="i-up-down">
    ...
    <NavFooter></NavFooter>
    <Modal v-show="mdShowConfirm" :mdShow="mdShowConfirm" @close="closeModal">
      <p slot="message">你确定要删除此地址吗？</p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delAddress">确认</a>
        <a class="btn btn--m" href="javascript:;" @click="mdShowConfirm = false">取消</a>
      </div>
    </Modal>
    <Modal v-show="mdShowAddConfirm" :mdShow="mdShowAddConfirm" @close="closeAddModal">
      <p slot="message">新增地址:</p>
      <div slot="btnGroup">
        <ul>
          <li class="regi_form_input">
            <i class="icon"></i>
            <input type="text" tabindex="1" v-model="peopleName" name="peopleName" class="regi_login_input regi_login_input_left" placeholder="收件人姓名" data-type="peopleName">
          </li>
          <li class="regi_form_input">
            <i class="icon"></i>
            <input type="text" tabindex="1" v-model="peopleTel" name="peopleTel" class="regi_login_input regi_login_input_left" placeholder="收件人电话" data-type="peopleTel">
          </li>
          <li class="regi_form_input">
            <i class="icon"></i>
            <input type="text" tabindex="1" v-model="peopleCode" name="peopleCode" class="regi_login_input regi_login_input_left" placeholder="地址邮编" data-type="peopleCode">
          </li>
          <li class="regi_form_input">
            <i class="icon"></i>
            <input type="text" tabindex="1" v-model="peopleAddress" name="peopleAddress" class="regi_login_input regi_login_input_left" placeholder="收件人地址" data-type="peopleAddress">
          </li>
        </ul>
        <a class="btn btn--m" href="javascript:;" @click="addAddressClick">确认</a>
        <a class="btn btn--m" href="javascript:;" @click="mdShowAddConfirm = false; peopleName= ''; peopleTel = ''; peopleAddress= '';">取消</a>
      </div>
    </Modal>
  </div>
</template>
<script>
  ...
  export default{
    ...
    computed: {
      // 使用计算属性默认展示3个地址
      listDataFilter() {
        return this.listData.slice(0, this.limit);
      }
    },
    components: {
      Modal,
      ...
      addAddressClick() {
        ...
        axios.post("/users/addAddress", {
          ...
          if(res.data.status == 0) {
            this.init();
            this.limit = this.listData.length + 1;
          }else {
            alert(res.data.msg);
          }
          this.closeAddModal();
          ...
          if(res.data.status == 0) {
            ...
            this.listData.forEach((item,index) => {
              if(item.isDefault == true) {
                ...
                console.log(this.selectAddressId, item)
              }
            })
          }
        })
      },
      // 使用计算属性改变数据内容多少, 以达到展开隐藏功能
      expand() {
        if(this.limit == 3) {
          this.limit = this.listData.length;
        }else {
          this.limit = 3;
        }
      },
