              ...
              <li v-for="(item, index) in dataList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" @click="productNumChange(item,2)" :class="{'check': item.checked == '1'}">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="productNumChange(item, 0)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="productNumChange(item, 1)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  export default{
    ...
    methods: {
      ...
      delCart() {
        ...
        axios.post('/users/cardDel',parmas).then((res) => {
      ...
      // 获取购物车列表
      getCartList() {
        axios.get('/users/cardList').then((res) => {
          if(res.data.status == 0) {
      ...
      productNumChange(item, type) {
        ...
        axios.post("/users/cardEdit", parmas).then((res) => {
          if(res.data.status == 0) {
          }else {
            alert(res.data.msg)
          }
